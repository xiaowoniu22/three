...
function BlockSpinner({ position = [0, 0, 0] }) {
  ...
  console.log(444, speed);
  useFrame((state) => {
    const time = state.clock.getElapsedTime()
    console.log(3333, time);
    
    const rotation = new THREE.Quaternion()
    console.log(1111, rotation);
    rotation.setFromEuler(new THREE.Euler(0, time * speed, 0))
    console.log(222, rotation);
...
function BlockAxe({ position = [0, 0, 0] }) {
  ...
  return <group position={position}>
    <mesh geometry={boxGeometry} material={floor2Material} position={[0, - 0.1, 0]} scale={[4, 0.2, 4]} receiveShadow />
    <RigidBody ref={obstacle} type="kinematicPosition" position={[0, 0.3, 0]} restitution={0.2} friction={0}>
      <mesh geometry={boxGeometry} material={obstacleMaterial} scale={[1.5, 1.3, 0.3]} castShadow receiveShadow />
    </RigidBody>
  </group>
}
...
export default function Level({ count = 5, types = [BlockSpinner, BlockAxe, BlockLimbo], seed = 0 }) {
  ...
  return <>
    ...
    {/* <Plane position={[-2, -1, -1]} rotation={[-Math.PI / 2, 0, 0]} /> */}
